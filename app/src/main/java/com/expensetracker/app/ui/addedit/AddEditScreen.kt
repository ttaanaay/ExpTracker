...
fun AddEditScreen(
    ...
    Scaffold(
        ...
        Column(
            ...
            CategoryInfoRow(category = state.selectedCategory)
...
@Composable
private fun CategoryInfoRow(category: CategoryEntity?) {
    Row(
        modifier = Modifier
            .fillMaxWidth()
        ...
        Text("หมวดหมู่", style = MaterialTheme.typography.bodyMedium)
        Row(verticalAlignment = Alignment.CenterVertically) {
            if (category != null) {
                CategoryIconBadge(icon = category.icon, color = Color(category.color), size = 28.dp)
                Spacer(modifier = Modifier.width(8.dp))
                Text(category.name, style = MaterialTheme.typography.bodyMedium, fontWeight = FontWeight.Medium)
            } else {
                Text("เลือกหมวดหมู่", style = MaterialTheme.typography.bodyMedium, fontWeight = FontWeight.Medium)
            }
            Icon(
                Icons.Filled.ChevronRight,
                contentDescription = null,
